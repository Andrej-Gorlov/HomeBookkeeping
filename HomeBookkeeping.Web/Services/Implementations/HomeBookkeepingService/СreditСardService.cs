...
namespace HomeBookkeeping.Web.Services.Implementations.HomeBookkeepingService
{
    public class СreditСardService : BaseService, IСreditСardService
    {
        ...
        public async Task<T> GetСreditСardsAsync<T>(PagingParameters parameters)
        {
            return await this.SendAsync<T>(new ApiRequest()
            {
                Api_Type = StaticDitels.ApiType.GET,
                Url = StaticDitels.HomeBookkeepingApiBase + "/api/creditcards?pagenumber=" + parameters.PageNumber + "&pagesize=" + parameters.PageSize
            });
        }
